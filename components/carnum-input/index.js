...
Component({
    ...
    observers: {
        ...
        'plateNum': function () {
            ...
            this.setData({
                canSubmit: !/\S+\s+\S+/.test(plateNum) && plateNum.replace(/\s*/g, '').length >= 7
            })
        }
    },
    methods: {
        ...
        init: function (options) {
            ...
            this.setData({
                plateNum: _plateNum,
                activeIndex: !_plateNum.length ? 0 : 
                    _plateNum.length > 7 ? 7 : 
                        _plateNum.length === 7 ? 6 : _plateNum.length
            })
            this._show()
        ...
        onClosePop() {
            IptCancelFunc && IptCancelFunc({
                plateNum: this.data.plateNum
            })
            this._close()
        },
        confirm() {
            IptCompleteFunc && IptCompleteFunc({
                plateNum: this.data.plateNum,
                callback: () => {
                    this._close()
                }
        ...
        handleInput({ currentTarget }) {
            ...
            let activeIndex = process === 7 ? 7 :
                process + 1 > 6 ? 6 :
                    (plateNum[process + 1] && plateNum[process + 1] !== ' ') ? process : process + 1
            this.setData({
                plateNum,
                activeIndex
            })
        ...
        getKeyBoardList() {
            ...
            if (keyboardType === 1) {
                this.setData({
                    iptKeyList: ProvinceList.map(e => ({ key: e, disabled: this.checkProvinceDisabled(e) }))
                })
            }
            ...
            if (keyboardType === 2) {
                this.setData({
                    iptKeyList: NumberList.concat(LetterList).map(e => ({ key: e, disabled: this.checkCityDisabled(e) }))
                })
            }
            ...
            if (keyboardType === 3) {
                this.setData({
                    iptKeyList: NumberList.concat(LetterList).map(e => ({ key: e, disabled: e === 'I' || e === 'O' }))
                })
            }
            ...
            if (keyboardType === 4) {
                ...
                this.setData({
                    iptKeyList: iptKeyList.map(e => ({ key: e, disabled: e === 'I' || e === 'O' })),
                    showAdditionList: false
                })
            }
